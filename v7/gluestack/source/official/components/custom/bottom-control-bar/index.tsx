...
export interface BottomControlBarProps {
  ...
  /** Optional host navigation override for a selected component */
  onComponentNavigate?: (component: ComponentItem) => void;
    ...
    const handleComponentSelect = useCallback(
      (component: ComponentItem, index: number) => {
        ...
        if (onComponentNavigate) {
          onComponentNavigate(component);
        } else if (component.path.includes('showcase')) {
          router.push(`/(home)/showcases/${component.path}` as any);
        } else {
          router.push(`/(home)/components/${component.path}` as any);
        }
      },
      [onComponentNavigate, router, pathname]
    );
