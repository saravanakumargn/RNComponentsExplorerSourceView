...
type ComponentsTabProps = {
  onComponentNavigate?: (path: string) => void;
};

export default function ComponentsTab({ onComponentNavigate }: ComponentsTabProps) {
  ...
  const handleCardPress = useCallback(
    (path: string) => {
      ...
      if (onComponentNavigate) {
        onComponentNavigate(path);
        return;
      }

      router.push(`/(home)/components/${path}` as any);
    },
    [onComponentNavigate, router]
  );
  ...
  return (
    <View className="flex-1 bg-background">
      ...
      <BottomControlBar
        ...
        onComponentNavigate={(component) => handleCardPress(component.path)}
      />
      <Animated.FlatList
        ref={listRef}
        data={components}
