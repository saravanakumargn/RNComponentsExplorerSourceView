...
type ShowcasesTabProps = {
  onShowcaseNavigate?: (path: string) => void;
};

export default function ShowcasesTab({ onShowcaseNavigate }: ShowcasesTabProps) {
  ...
  const handleCardPress = useCallback(
    (path: string) => {
      ...
      if (onShowcaseNavigate) {
        onShowcaseNavigate(path);
        return;
      }

      router.push(`/(home)/showcases/${path}` as any);
    },
    [onShowcaseNavigate, router]
  );
  ...
  return (
    <View className="flex-1 bg-background">
      ...
      <BottomControlBar
        ...
        onComponentNavigate={(showcase) => handleCardPress(showcase.path)}
      />
    </View>
  );
}
